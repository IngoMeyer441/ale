...
call ale#Set('cpp_clangunix_options', '-std=c11 -Wall')
call ale#Set('cpp_clangunix_unix_include_directory', expand('~/linux_include'))
...
function! ale_linters#cpp#clangunix#GetCommand(buffer) abort
    ...
    \   . (!empty(ale#Var(a:buffer, 'cpp_clangunix_unix_include_directory')) ? '-nostdinc -isystem' . ale#Var(a:buffer, 'cpp_clangunix_unix_include_directory') . ' ' : '')
    \   . ' -'
endfunction
